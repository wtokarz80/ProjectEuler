...
public class LargestProduct {
    ...
    public static long findMaxValue(String number) {
        List<Long> multipliedNumbers = new ArrayList<>();
//        String str = number.replace(" ", "");
        ...
        for (int i = 0; i <= numbers.length-13; i++){
            ...
            for (int j = 0; j < 13; j++){
                multipliedNumber *= numbers[i+j];
            }
            multipliedNumbers.add(multipliedNumber);
        }
        multipliedNumbers.sort(Collections.reverseOrder());
        return multipliedNumbers.get(0);
    }
